...
import {
  RoleAuth,
  RoleAuthCQRS,
} from "../../../../src/context/shared/decorator/roleAuthorization.decorator";
...
import { AuthorizedCommand } from "../../../../src/context/shared/domain/authorizedCommand";

describe("RoleAuth Decorator", () => {
  describe("for service RoleAuth", () => {
    class TestClass {
      @RoleAuth([Role.Admin])
  ...
  describe("for RoleAuthCQRS", () => {
    class TestClass {
      @RoleAuthCQRS([Role.Admin])
      async adminOnlyMethod(command: AuthorizedCommand) {
        return "admin access granted";
      }

      @RoleAuthCQRS([Role.Client, Role.Musician])
      async clientOrMusicianMethod(command: AuthorizedCommand) {
        return "client or musician access granted";
      }
    }
    ...
    describe("adminOnlyMethod", () => {
      it("should allow access when user has Admin role", async () => {
        ...
        const result = await testInstance.adminOnlyMethod({
      ...
      it("should throw UnauthorizedRoleException when user has non-Admin role", async () => {
        ...
        await expect(async () => {
          await testInstance.adminOnlyMethod({
            get authorized(): UserAuthInfo {
              return authInfo;
            },
          });
    ...
    describe("clientOrMusicianMethod", () => {
      it("should allow access when user has Client role", async () => {
        ...
        const result = await testInstance.clientOrMusicianMethod({
          get authorized(): UserAuthInfo {
            return authInfo;
          },
        });
      ...
      it("should throw UnauthorizedRoleException when user has unauthorized role", async () => {
        ...
        await expect(async () => {
          await testInstance.clientOrMusicianMethod({
            get authorized(): UserAuthInfo {
              return authInfo;
            },
          });
